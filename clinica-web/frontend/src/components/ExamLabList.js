...
function ExameLabList() {
  ...
  const deletarExame = async (id) => {
    if (window.confirm('Tem certeza que deseja excluir este exame laboratorial?')) {
      try {
  ...
  return (
    <div>
        ...
        <tbody>
          {exames.map(exame => (
            <tr key={exame.id}>
              ...
              <td>
                {exame.atendimento 
                  ? `Atendimento #${exame.atendimento.id} — ${exame.atendimento.data}` 
                  : '-'}
              </td>
              <td>
      ...
      {exames.length === 0 && <p className="empty">Nenhum exame laboratorial cadastrado.</p>}
    </div>
  );
}
